                    ...
                    <div class="shadow-sm ring-1 ring-black ring-opacity-5">
                        <table class="min-w-full border-separate" style="border-spacing: 0">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="sticky top-0 z-10 border-b border-gray-300 bg-gray-50 bg-opacity-75 px-3 py-3.5 text-left text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter sm:table-cell">Published On</th>
                            ...
                            @foreach($articles as $article)
                                <tr>
                                    <td class="whitespace-nowrap border-b border-gray-200 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 lg:pl-8">
                                        {{ $article->title }}</td>
                                    <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-sm text-gray-500 sm:table-cell">{{ $article->published_at ? $article->published_at->format('jS M Y H:i') : 'Not Published' }}</td>
                                    ...
                                    <td class="relative whitespace-nowrap border-b border-gray-200 py-4 pr-4 pl-3 text-right text-sm font-medium sm:pr-6 lg:pr-8">
                                        <a href="{{ route('admin.articles.edit', ['article' => $article]) }}" class="text-indigo-600 hover:text-indigo-900">Edit<span class="sr-only">, {{ $article->title }}</span></a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
