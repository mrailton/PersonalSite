...
test('a visitor can not view an article that has not been published yet', function () {
    $unpublishedArticle = Article::factory()->unpublished()->create();

    test()->get(route('articles.show', ['article' => $unpublishedArticle]))
        ->assertStatus(404)
