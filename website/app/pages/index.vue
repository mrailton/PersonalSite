...
const {data: articles, pending, error} = await useAsyncData('latest-articles', () =>
    $fetch(`${config.public.apiUrl}/articles?limit=3`)
)
</script>
...
<template>
  <div>
    <section class="bg-white dark:bg-gray-900">
      <div class="max-w-3xl px-6 text-center mx-auto">
        <h2 class="text-3xl font-semibold text-gray-800 dark:text-gray-100">
          Hi there,
          <span class="bg-indigo-600 text-white px-1 dark:bg-indigo-500">
            I'm Mark
          </span>.
        </h2>
        <NuxtImg src="/img/headshot.jpg" alt="Mark Railton" class="w-48 mx-auto my-8 rounded-full"/>
        <p class="text-gray-600 dark:text-gray-300 mt-8 mb-8">
          I'm a software developer living in the beautiful County Wicklow, Ireland focusing on building web applications
          using PHP, Python and Vue.js.
        </p>
      </div>
    </section>

    <section class="bg-gray-800 dark:bg-gray-900 py-10 pattern">
      <div class="max-w-5xl px-6 mx-auto text-center">
        <h2 class="text-2xl font-semibold text-white dark:text-gray-100">About Me</h2>
        <p class="text-white dark:text-gray-300 mt-4">
        ...
        <p class="text-white dark:text-gray-300 mt-4">
          ...
          Currently, it's built using NuxtJS with TailwindCSS for the website with a Python API built with Flask.
        </p>
      </div>
    </section>
    ...
    <section class="bg-white dark:bg-gray-900 py-20">
      <div class="max-w-5xl px-6 mx-auto text-center">
        <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Latest Blog Articles</h2>
        ...
        <div v-else>
          <div v-if="!articles || !articles.length" class="text-gray-500 mt-6">No articles available.</div>
          <div v-else>
            <div v-for="article in articles" :key="article.id" class="flex flex-col items-center justify-center mt-6">
        ...
        <div class="flex items-center justify-center mt-4">
          <NuxtLink to="/blog"
                    class="px-6 py-3 bg-gray-800 text-white rounded-md hover:bg-gray-700 dark:bg-gray-200 dark:text-gray-900 dark:hover:bg-gray-300">
            <span>View More</span>
          </NuxtLink>
        </div>
      </div>
